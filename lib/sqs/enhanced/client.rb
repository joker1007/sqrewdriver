...
module SQS
  module Enhanced
    class Client
      ...
      def initialize(queue_url:, client: nil, threads: 32, serializer: SQS::Enhanced.default_serializer, aggregate_messages_per: nil, flush_retry_count: 5, **options)
        if client
          @client = client
        else
      ...
      def ensure_serializer_for_aggregation!(serializer)
        valid_serializer = @aggregate_messages_per.nil? || serializer.is_a?(SQS::Enhanced::Serdes::JSONSerde)
